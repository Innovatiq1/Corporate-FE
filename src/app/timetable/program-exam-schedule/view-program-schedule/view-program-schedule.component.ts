...
@Component({
  selector: 'app-view-program-schedule',
  templateUrl: './view-program-schedule.component.html',
  styleUrls: ['./view-program-schedule.component.scss']
})
export class ViewProgramScheduleComponent {

  breadscrums = [
    {
      title: 'Edit Exam-Schedule',
      items: ['Program Exam Schedule'],
      active: 'View Program Exam Schedule',
    },
  ];
  ...
  loadData(){
    this.examScheduleService.getProgramExamById(this.examId).subscribe((response:any)=>{
...
delete(row: ExamSchedule) {
   Swal.fire({
     ...
     if (result.isConfirmed) {
       this.examScheduleService.deleteprogramExam(row.id).subscribe(
         () => {
           Swal.fire({
             title: "Deleted",
