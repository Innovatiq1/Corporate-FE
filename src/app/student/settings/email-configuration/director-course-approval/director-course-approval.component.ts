...
@Component({
  selector: 'app-director-course-approval',
  templateUrl: './director-course-approval.component.html',
  styleUrls: ['./director-course-approval.component.scss']
})
export class DirectorCourseApprovalComponent {
  directorCourseApprovalForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  constructor(
     ...
     {
    this.directorCourseApprovalForm = this.formBuilder.group({
  ...
  ngOnInit(){
    this.getDirectorTrainingReqApproval();
  }
  fetchUpdated() {
  ...
  patchForm(pageContent: any) {
    ...
    this.directorCourseApprovalForm.patchValue({
  ...
  update(){
    if (this.directorCourseApprovalForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.directorCourseApprovalForm.markAllAsTouched(); 
