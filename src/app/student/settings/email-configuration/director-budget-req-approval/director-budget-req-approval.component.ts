...
@Component({
  selector: 'app-director-budget-req-approval',
  templateUrl: './director-budget-req-approval.component.html',
  styleUrls: ['./director-budget-req-approval.component.scss']
})
export class DirectorBudgetReqApprovalComponent {
  budgetReqApprovalForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  breadscrums = [
    {
      ...
      active: 'Budget Request Approval',
    },
  ];
  ...
  constructor(
     ...
     {
    this.budgetReqApprovalForm = this.formBuilder.group({
  ...
  getBudgetReqApproval() {
    this.emailConfigurationService.getForgetPasswordTemplate().subscribe( response =>{
      this.assignData  = response?.data?.docs[0]?.budget_approval_template;
  ...
  update(){
    if (this.budgetReqApprovalForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.budgetReqApprovalForm.markAllAsTouched(); 
