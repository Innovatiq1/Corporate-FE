...
@Component({
  selector: 'app-ro-course-rejected',
  templateUrl: './ro-course-rejected.component.html',
  styleUrls: ['./ro-course-rejected.component.scss']
})
export class RoCourseRejectedComponent {
  roCourseRejectedForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  breadscrums = [
    {
      ...
      active: 'Approver 1 Training Request Rejected',
    },
  ];
  ...
  constructor(
     ...
     {
    this.roCourseRejectedForm = this.formBuilder.group({
  ...
  patchForm(pageContent: any) {
    ...
    this.roCourseRejectedForm.patchValue({
  ...
  getRoTrainingReqRejected() {
    this.emailConfigurationService.getForgetPasswordTemplate().subscribe( response =>{
      this.assignData  = response?.data?.docs[0]?.ro_course_rejected_template;
  ...
  update(){
    if (this.roCourseRejectedForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.roCourseRejectedForm.markAllAsTouched(); 
    this.isSubmitted = true;
  }
  }
  updateTemplate() {
    return new Promise<void>((resolve, reject) => {
          const obj = this.roCourseRejectedForm.value;
          obj.insertaction = 'ro_course_rejected_template';
