...
@Component({
  selector: 'app-ta-course-approval',
  templateUrl: './ta-course-approval.component.html',
  styleUrls: ['./ta-course-approval.component.scss']
})
export class TaCourseApprovalComponent {
  taCourseApprovalForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  breadscrums = [
    {
      ...
      active: 'Approver 3 Training Request Approval',
    },
  ];
  ...
  constructor(
     ...
     {
    this.taCourseApprovalForm = this.formBuilder.group({
  ...
  update(){
    if (this.taCourseApprovalForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.taCourseApprovalForm.markAllAsTouched(); 
    this.isSubmitted = true;
  }
  }
  updateTemplate() {
    return new Promise<void>((resolve, reject) => {
          const obj = this.taCourseApprovalForm.value;
          obj.insertaction = 'trainingadmin_course_approved_template';
