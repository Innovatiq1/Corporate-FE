...
@Component({
  selector: 'app-dept-budget-req',
  templateUrl: './dept-budget-req.component.html',
  styleUrls: ['./dept-budget-req.component.scss']
})
export class DeptBudgetReqComponent {
  deptBudgetReqForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  constructor(
     ...
     {
    this.deptBudgetReqForm = this.formBuilder.group({
  ...
  patchForm(pageContent: any) {
    ...
    this.deptBudgetReqForm.patchValue({
  ...
  getDeptBudgetReq() {
    this.emailConfigurationService.getForgetPasswordTemplate().subscribe( response =>{
      this.assignData  = response?.data?.docs[0]?.dept_budget_requested_template;
  ...
  update(){
    if (this.deptBudgetReqForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.deptBudgetReqForm.markAllAsTouched(); 
    this.isSubmitted = true;
  }
  }
  updateTemplate() {
    return new Promise<void>((resolve, reject) => {
          const obj = this.deptBudgetReqForm.value;
          obj.insertaction = 'dept_budget_requested_template';
