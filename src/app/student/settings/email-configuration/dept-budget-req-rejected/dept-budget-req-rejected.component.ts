...
@Component({
  selector: 'app-dept-budget-req-rejected',
  templateUrl: './dept-budget-req-rejected.component.html',
  styleUrls: ['./dept-budget-req-rejected.component.scss']
})
export class DeptBudgetReqRejectedComponent {
  deptBudgetReqRejectedForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  breadscrums = [
    {
      ...
      active: 'Department Budget Request Rejected',
    },
  ];
  ...
  constructor(
     ...
     {
    this.deptBudgetReqRejectedForm = this.formBuilder.group({
  ...
  getDeptBudgetReqRejected() {
    this.emailConfigurationService.getForgetPasswordTemplate().subscribe( response =>{
      this.assignData  = response?.data?.docs[0]?.dept_budget_rejected_template;
  ...
  update(){
    if (this.deptBudgetReqRejectedForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.deptBudgetReqRejectedForm.markAllAsTouched(); 
