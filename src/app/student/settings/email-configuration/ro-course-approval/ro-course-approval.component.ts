...
@Component({
  selector: 'app-ro-course-approval',
  templateUrl: './ro-course-approval.component.html',
  styleUrls: ['./ro-course-approval.component.scss']
})
export class RoCourseApprovalComponent {
  roCourseApprovalForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  breadscrums = [
    {
      ...
      active: 'Approver 1 Training Request Approval',
    },
  ];
  ...
  constructor(
     ...
     {
    this.roCourseApprovalForm = this.formBuilder.group({
  ...
  patchForm(pageContent: any) {
    ...
    this.roCourseApprovalForm.patchValue({
  ...
  getRoTrainingReqApproval() {
    this.emailConfigurationService.getForgetPasswordTemplate().subscribe( response =>{
      this.assignData  = response?.data?.docs[0]?.ro_course_approved_template;
  ...
  update(){
    if (this.roCourseApprovalForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.roCourseApprovalForm.markAllAsTouched(); 
    this.isSubmitted = true;
  }
  }
  updateTemplate() {
    return new Promise<void>((resolve, reject) => {
          const obj = this.roCourseApprovalForm.value;
          obj.insertaction = 'ro_course_approved_template';
