...
@Component({
  selector: 'app-director-budget-req-rejected',
  templateUrl: './director-budget-req-rejected.component.html',
  styleUrls: ['./director-budget-req-rejected.component.scss']
})
export class DirectorBudgetReqRejectedComponent {
  budgetRequestRejectedForm:FormGroup  ;
  edit = true;
  welcomeUrl: any;
  trainerUrl: any;
  rejectUrl: any;  
  ...
  breadscrums = [
    {
      ...
      active: 'Budget Request Rejected',
    },
  ];
  ...
  constructor(
     ...
     {
    this.budgetRequestRejectedForm = this.formBuilder.group({
  ...
  update(){
    if (this.budgetRequestRejectedForm.valid) {
    Swal.fire({
      title: 'Are you sure?',
    ...
    this.budgetRequestRejectedForm.markAllAsTouched(); 
    this.isSubmitted = true;
  }
  }
  updateTemplate() {
    return new Promise<void>((resolve, reject) => {
          const obj = this.budgetRequestRejectedForm.value;
          obj.insertaction = 'budget_rejected_template';
